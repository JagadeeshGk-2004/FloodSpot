...
export default function App() {
  ...
  // Community Report Verification State in App portal
  ...
  // Filtered reports for active view
  const activeNonResolvedReports = reports.filter(r => 
    r.status !== 'resolved' && r.active !== false && r.is_resolved !== true
  );
  ...
  return (
    <div className="relative w-screen h-screen bg-slate-950 text-slate-100 overflow-hidden flex flex-col font-sans select-none">
      ...
      {activeUserSOS && (
        <div className="relative z-40 px-4 py-2 bg-gradient-to-r from-red-700 via-rose-700 to-red-800 text-white border-b border-red-400/40 shadow-2xl flex items-center justify-between pointer-events-auto animate-in slide-in-from-top duration-300">
          ...
          <button
            ...
            className="px-3 py-1 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-extrabold text-xs shadow-lg flex items-center gap-1 cursor-pointer shrink-0 transition-transform active:scale-95 border border-emerald-300"
          >
        ...
        <div className={`fixed ${activeUserSOS ? 'top-28' : alertBannerConfig ? 'top-24' : 'top-16'} left-4 right-4 z-50 max-w-md mx-auto pointer-events-auto animate-in slide-in-from-top duration-300`}>
          <div className={`glass-panel p-3.5 rounded-2xl border shadow-2xl flex items-center justify-between gap-3 text-xs font-semibold ${
            ...
            <div className="flex items-center gap-2">
              {toastNotification.type === 'success' ? <ShieldCheck className="w-4 h-4 text-emerald-400 shrink-0" /> :
               toastNotification.type === 'offline' ? <WifiOff className="w-4 h-4 text-amber-400 shrink-0" /> : 
               <Wifi className="w-4 h-4 text-cyan-400 shrink-0" />}
              <span>{toastNotification.text}</span>
            </div>
            <button onClick={() => setToastNotification(null)} className="text-slate-400 hover:text-slate-200 px-1 font-bold">✕</button>
          </div>
        </div>
      )}
      ...
      <header className={`absolute ${activeUserSOS ? 'top-20' : alertBannerConfig ? 'top-12' : 'top-4'} left-4 right-4 z-20 flex items-center justify-between pointer-events-none transition-all duration-300`}>
        ...
        <div className="flex items-center gap-2.5 pointer-events-auto">
          <button
            ...
            className="p-2.5 rounded-2xl glass-panel border border-slate-700/80 hover:border-cyan-500/50 text-slate-200 hover:text-cyan-400 transition-all cursor-pointer shadow-lg active:scale-95 flex items-center justify-center shrink-0"
          ...
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-2xl bg-gradient-to-tr from-cyan-500 to-blue-600 p-0.5 shadow-lg shadow-cyan-500/30 shrink-0">
              <div className="w-full h-full bg-slate-950 rounded-[14px] flex items-center justify-center text-cyan-400">
                <Droplet className="w-4.5 h-4.5 fill-cyan-400" />
              </div>
            </div>
            <div>
              <h1 className="font-extrabold text-base tracking-tight text-white flex items-center gap-1.5 font-['Outfit']">
                FloodSpot
                {isOnline ? (
              ...
              <p className="text-[10px] text-slate-400 hidden sm:block">Real-Time Flood Radar (India)</p>
            </div>
          </div>
        </div>
      ...
      <main className="relative flex-1 w-full h-full">
        ...
        {activeTab === 'routes' && (
          <div className="w-full h-full pt-24 pb-28 px-4 overflow-y-auto max-w-xl mx-auto space-y-4 no-scrollbar">
            <div className="glass-panel p-5 rounded-3xl border border-slate-700/60 shadow-xl space-y-3">
              <div className="flex items-center justify-between">
                <h2 className="font-bold text-lg text-slate-100 flex items-center gap-2">
                  <Navigation className="w-5 h-5 text-cyan-400" />
                  Safe Travel Routes & Advisories
                </h2>
                <span className="text-xs text-emerald-400 bg-emerald-950/80 border border-emerald-800 px-2 py-0.5 rounded-full font-semibold">
              ...
              <div className="space-y-3 pt-2">
                <div className="p-3.5 rounded-2xl bg-slate-900/80 border border-emerald-500/30 flex items-start justify-between">
                  <div>
                    <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">Recommended Route</span>
                    <h4 className="font-bold text-sm text-slate-100">OMR Highway Corridor</h4>
                    <p className="text-xs text-slate-400">Guindy → Taramani → Siruseri</p>
                    <p className="text-[11px] text-emerald-300 mt-1">Clear of standing water. Safe speed flow.</p>
                  </div>
                  <span className="text-xs font-bold text-emerald-400 bg-emerald-950 px-2 py-1 rounded-lg">CLEAR</span>
                </div>

                <div className="p-3.5 rounded-2xl bg-slate-900/80 border border-red-500/30 flex items-start justify-between">
                  <div>
                    <span className="text-[10px] font-bold text-red-400 uppercase tracking-wider">Blocked Route</span>
                    <h4 className="font-bold text-sm text-slate-100">Velachery Main Road & Subway</h4>
                    <p className="text-xs text-slate-400">Tambaram ←→ Guindy Bypass</p>
                    <p className="text-[11px] text-red-300 mt-1">Submerged under subway (&gt;2.5 ft water). Avoid completely.</p>
                  </div>
                  <span className="text-xs font-bold text-red-400 bg-red-950 px-2 py-1 rounded-lg">BLOCKED</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full h-full pt-24 pb-28 px-4 overflow-y-auto max-w-xl mx-auto space-y-3 no-scrollbar">
            <div className="flex items-center justify-between">
              ...
              {myReportsOnly && (
                <button
                  ...
                  className="text-xs font-semibold text-cyan-400 hover:text-cyan-300 bg-cyan-950/60 border border-cyan-800 px-2.5 py-1 rounded-xl flex items-center gap-1 cursor-pointer"
                >
            ...
            {weatherData && (
              <div className={`p-4 rounded-2xl border shadow-xl flex items-start gap-3 ${
                rain1h > 7.5 ? 'bg-red-950/80 border-red-500/80 text-red-200' :
                rain1h >= 2.5 ? 'bg-orange-950/80 border-orange-500/80 text-orange-200' :
                'bg-slate-900/90 border-cyan-500/40 text-cyan-200'
              }`}>
                <CloudRain className="w-6 h-6 text-cyan-400 shrink-0 mt-0.5" />
                <div className="space-y-1 w-full">
                  <div className="flex items-center justify-between">
                    <span className="font-extrabold text-sm text-white">Live Weather Status — {weatherData.cityName || 'Chennai'}</span>
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-slate-950 text-cyan-300 border border-slate-700">
            ...
            {displayedReports.length === 0 ? (
              <div className="glass-panel p-8 rounded-3xl border border-slate-700/60 text-center space-y-2">
                <FileText className="w-8 h-8 text-slate-500 mx-auto" />
                <p className="font-semibold text-slate-300">No active incident reports found.</p>
              </div>
            ) : (
              displayedReports.map((report) => (
                <div 
                  key={report.id}
                  onClick={() => { setSelectedReport(report); setActiveTab('map'); }}
                  className="glass-panel p-4 rounded-2xl border border-slate-700/60 shadow-lg hover:border-cyan-500/50 transition-all cursor-pointer space-y-2"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase ${
                        ...
                        report.severity === 'medium' ? 'bg-amber-400 text-slate-950' :
                      ...
                      {report.full_name && (
                        <span className="text-[10px] font-medium text-slate-400">
                          by <strong className="text-slate-300">{report.full_name}</strong>
                        </span>
                      )}
                    </div>
                  ...
                  {report.water_depth && (
                    <div className="text-xs text-cyan-300 font-semibold flex items-center gap-1">
                      <Droplet className="w-3.5 h-3.5 text-cyan-400 fill-cyan-400" />
                      Water Depth: {report.water_depth}
                    </div>
                  )}
                </div>
              ))
        ...
        {activeTab === 'verify' && (
          <div className="w-full h-full pt-24 pb-28 px-4 overflow-y-auto max-w-xl mx-auto space-y-4 no-scrollbar">
            <div className="glass-panel p-5 rounded-3xl border border-slate-700/60 shadow-xl space-y-3">
              ...
              <p className="text-xs text-slate-300">
                Help verify crowdsourced waterlogged area reports uploaded by citizens using Algorithmic Inspection + Community Review.
              </p>
              
              <div className="p-3.5 rounded-2xl bg-slate-900/80 border border-slate-700/80 flex items-center gap-2 text-cyan-400 text-xs font-semibold">
                <CheckCircle2 className="w-4 h-4 shrink-0" />
                <span>Visual Feature Analysis Active — Automated Feature Match Scoring Enabled</span>
              </div>
            </div>

            {/* List of Reports for Community Verification */}
            <div className="space-y-3">
              <h3 className="font-bold text-sm text-slate-200 flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-400 font-normal">Click to vote</span>
              </h3>

              {displayedReports.length === 0 ? (
                <div className="glass-panel p-6 rounded-2xl text-center text-slate-400 text-xs">
                  No reports pending community verification.
                </div>
              ) : (
                displayedReports.map((report) => {
                  const vData = verifications[report.id] || { upvotes: 0, flags: 0, voted: null };
                  return (
                    <div 
                      key={report.id}
                      className="glass-panel p-4 rounded-2xl border border-slate-700/60 space-y-2.5 shadow-lg"
                    >
                      <div className="flex items-center justify-between">
              ...
              <div className="glass-panel p-4 rounded-2xl border border-slate-700/60 space-y-2">
                <h4 className="font-bold text-xs text-amber-300 flex items-center gap-1.5">
                  <PhoneCall className="w-4 h-4" />
                  Emergency Helplines
                </h4>
                <ul className="text-xs text-slate-300 space-y-1 font-mono">
                  <li>Flood Control Helpline: <strong>1913</strong></li>
                  <li>Disaster Response: <strong>1070</strong></li>
                  <li>Emergency Fire & Rescue: <strong>101</strong></li>
                </ul>
              </div>
            </div>
          </div>
      ...
      {showResolveConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="glass-panel p-6 rounded-3xl border border-emerald-500/40 shadow-2xl max-w-sm w-full space-y-4 text-center">
            <div className="w-12 h-12 rounded-2xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 flex items-center justify-center mx-auto shadow-lg">
              <ShieldCheck className="w-6 h-6" />
            </div>

            <div className="space-y-1">
              <h3 className="font-bold text-base text-slate-100">Confirm SOS Resolution</h3>
              <p className="text-xs text-slate-300 leading-relaxed">
            ...
            <div className="grid grid-cols-2 gap-2 pt-2">
              ...
              <button
                ...
                className="py-2.5 px-4 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs rounded-2xl cursor-pointer transition-all"
              >
                Cancel
              </button>
            </div>
